...
export default function XatPage() {
  ...
  const generarResposta = (pregunta: string, referencies: ReferenciaBibliografica[]): string => {
    ...
    if (preguntaLower.includes('qui') || preguntaLower.includes('autor') || preguntaLower.includes('ha escrit')) {
      ...
      const autorsDestacats = Array.from(new Set(bibliografia.flatMap(ref => ref.autors))).slice(0, 10);
      return `Alguns autors destacats de la bibliografia són: ${autorsDestacats.join(', ')}. Pots preguntar sobre un autor específic per veure les seves obres.`;
    }
